...
def start_replication_handshake(args: argparse.Namespace) -> None:
    ...
    with socket.create_connection((master_host, master_port)) as master_socket:
        ...
        commands = [
            resp.Array([resp.BulkString(str(Command.PING))]),
            resp.Array([
        ...
        for command in commands:
            logging.info(f"Sending {command} to master")
...
def init_server(args: argparse.Namespace) -> None:
    ...
    if args.replicaof:
        ...
        start_replication_handshake(args)
    else:
